...
namespace _2nd.Semester.Eksamen.Domain.Entities.Schedules.EmployeeSchedules
{
    public class ScheduleDay : BaseEntity
    {
        ...
        {
            return TimeRanges.Any(r => r.Type == TimeRangeType.Freetime && r.Duration >= duration);
        }

        public TimeRange? GetAvailableTimeRange(TimeSpan duration)
        {
            return TimeRanges.FirstOrDefault(r => r.Type == TimeRangeType.Freetime && r.Duration >= duration);
        }

        public IEnumerable<TimeRange> GetAllAvailableSlots(TimeSpan duration)
        {
            return TimeRanges.Where(r => r.Type == TimeRangeType.Freetime && r.Duration >= duration).ToList().OrderBy(r => r.Start);
        }
        ...
        public bool AddBooking(TreatmentBooking treatment)
        {
            /*
             ...
             */
            var booking = new TimeRange(TimeOnly.FromDateTime(treatment.Start), TimeOnly.FromDateTime(treatment.End)) { Name = treatment.Treatment.Name, Type = TimeRangeType.Booked, BookingID = treatment.BookingID};
            var free = TimeRanges.FirstOrDefault(r => r.Type == TimeRangeType.Freetime && r.Start <= booking.Start && r.End >= booking.End);
            if (free == null)
                return false;
            // before the booking
            ...
            if (free.Start < booking.Start) //if freetime starts before booking start then we need a freetime timerange before booking
                TimeRanges.Add(new TimeRange(free.Start,booking.Start) { Name = "Freetime", Type = TimeRangeType.Freetime});

            if (free.End > booking.End)//if freetime ends after booking end then we need a freetime timerange after booking
                TimeRanges.Add(new TimeRange(booking.End, free.End) { Name = "Freetime", Type = TimeRangeType.Freetime});
        ...
        public bool CancelBooking(TreatmentBooking treatment, int bookingId)
        {
            var _start = TimeOnly.FromDateTime(treatment.Start);
            var _end = TimeOnly.FromDateTime(treatment.End);
            var booking = TimeRanges.FirstOrDefault(r => r.Type == TimeRangeType.Booked &&
                                                          r.Start == _start &&
                                                          r.End == _end && r.BookingID == bookingId);
        ...
        public bool UpdateDaySchedule(TreatmentBooking treatment, int bookingId)
        {
            var _start = TimeOnly.FromDateTime(treatment.Start);
            var _end = TimeOnly.FromDateTime(treatment.End);
            var booking = TimeRanges.FirstOrDefault(r => r.Type == TimeRangeType.Booked &&
            ...
            if(_start > oldStart)
            {
                TimeRanges.Add(new TimeRange(oldStart, _start) { Type = TimeRangeType.Freetime });
            }
            if (_end < oldEnd)
            {
                TimeRanges.Add(new TimeRange(_end, oldEnd) { Type = TimeRangeType.Freetime });
            }
            MergeAdjacentFreetime();
            return true;
        }
